...
def plot_pert(pert_series, iterations):
    ...
    # print("pert.rvs(iterations) tpye is: ", type(pert.rvs(iterations)))
...
def read_document(file):
    ...
    for i in range(9, last_row):
        ...
        estimative = sheet.cell(row=i, column=2)
        distribution = sheet.cell(row=i, column=7)
        lower_value = sheet.cell(row=i, column=8)
        likely_value = sheet.cell(row=i, column=9)
        upper_value = sheet.cell(row=i, column=10)
        if estimative.value is None:
            break
        estimative_list.append(
            [row, id.value, estimative.value, distribution.value, lower_value.value, likely_value.value,
             upper_value.value])

    return estimative_list


# open sheet and insert risks
def write_risk_to_excel(planilha, row, result30, result60, result80):
    worksheet = planilha["Risk Matrix"]

    worksheet.cell(row=row, column=12).value = result30
    worksheet.cell(row=row, column=13).value = result60
    worksheet.cell(row=row, column=14).value = result80
